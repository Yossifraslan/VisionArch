...
router.post("/api/projects/rename", async ({ request, user }) => {
  try {
    ...
    if (!id || !name?.trim()) return jsonError(400, "id and name are required");

    const privateKey = `${PROJECT_PREFIX}${id}`;
    const privateProject = await userPuter.kv.get(privateKey);

    if (privateProject) {
      const updated = { ...privateProject, name: name.trim() };
      await userPuter.kv.set(privateKey, updated);
      return jsonOk({ project: updated });
    }

    const publicKey = `${PUBLIC_PROJECT_PREFIX}${id}`;
    const publicProject = await me.puter.kv.get(publicKey);

    if (publicProject && publicProject.ownerId === userId) {
      const updated = { ...publicProject, name: name.trim() };
      await me.puter.kv.set(publicKey, updated);
      return jsonOk({ project: updated });
    }

    return jsonError(404, "Project not found or not owned by you");
  } catch (e) {
